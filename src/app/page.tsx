...
export default function Home() {
  const { isAdmin, selectedCategory: storeCat, setCategory } = useOmniaStore();
  const router = useRouter();

  const [selectedCategory, setSelectedCategory] = useState<string | null>(storeCat.toLowerCase());
  ...
  return (
    <AppShell>
      <div className="min-h-full w-full flex flex-col items-center justify-center p-6 md:p-12 selection:bg-blue-500/30">
        {/* Background Glow */}
            ...
            {CATEGORIES.map((cat) => (
              <motion.button
                ...
                onClick={() => {
                  ...
                  // Sync with store
                  const storeKey = cat.name.split(' ')[0] as any;
                  setCategory(storeKey);
                }}
